        ...
        {error ? (
          <div className="rounded-2xl border border-red-500/20 bg-red-500/[0.04] p-6 text-center space-y-4">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 text-xs bg-white/[0.05] border border-white/[0.1] px-4 py-2 rounded-xl text-white hover:bg-white/[0.1] transition-all"
            >
          ...
          <div className="space-y-4">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="h-44 rounded-2xl bg-white/[0.03] border border-white/[0.06] animate-pulse"
              />
            ))}
          </div>
        ) : proposals.length === 0 ? (
          <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-12 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-[#9CA3AF] text-xs mt-1">
                When freelancers apply to your job posts, they will appear here.
              </p>
            </div>
          </div>
        ) : (
                ...
                return (
                  ...
                  >
                    {proposal.status !== "pending" && (
                      <div
                        className={`absolute top-0 right-0 px-4 py-1 text-[10px] font-bold uppercase rounded-bl-xl tracking-wider 
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-2 border-t border-white/[0.05]">
                        ...
                        {proposal.status === "pending" && (
                          <div className="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            {/* Accept & Pay — Stripe Checkout Form */}
                            {!isTaskClosed ? (
                              <form action="/api/checkout_sessions" method="POST">
                                <input type="hidden" name="proposalId" value={pId} />
                                <input type="hidden" name="amount" value={proposal.proposed_budget} />
                                <input type="hidden" name="taskId" value={proposal.taskId} />
                                <button
                                  type="submit"
                                  role="link"
                                  disabled={isProcessing}
                                  className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-white bg-[#7C6BF8] hover:bg-[#6C5CE7] transition-all shadow-md shadow-[#7C6BF8]/10 disabled:opacity-40"
                                >
                              ...
                              <button
                                disabled
                                className="inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold disabled:opacity-40 disabled:bg-neutral-800 disabled:text-neutral-500"
                              >
                                <Check size={13} /> Accept & Pay
                              </button>
                            )}
